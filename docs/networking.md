...
Note: frame counts come from `peek_waif_header` in session.rs, which inspects WAIF headers as they pass through (zero-copy). A "frame" here is a single 20ms WAIF streaming frame.
...
The cumulative values (`frames_expected`, `frames_received`, `late_frames`, `decode_failures`) support delta computation at phase transitions. The point-in-time values (`rtt_us`, `jitter_us`) are overwritten on each report. All new fields use `#[serde(default)]` for backward compatibility with older clients.
